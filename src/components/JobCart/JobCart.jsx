...
const JobCart = ({ jobCart }) => {
  ...
  return (
    <div>
      <div className="flex justify-between items-center border my-7 p-8 rounded-xl">
        <div className="flex gap-5 items-center">
          <div className="bg-[#F4F4F4] px-11 py-24 rounded-xl">
            <img src={logo} alt="" />
          </div>
          <div>
            <h2 className="text-6 font-extrabold text-[#474747]">{job_title}</h2>
            <h4 className=" text-xl font-bold text-[#757575] my-4">{company_name}</h4>
        ...
        <div>
          <button className="btn bg-[#7E90FE] text-[#FFFFFF] text-xl font-extrabold">View Details</button>
        </div>
      </div>
    </div>
  );
